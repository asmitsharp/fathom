...
export const CrewDashboard = () => {
  ...
  const { data: tasks = [] } = useQuery({
    queryKey: ["crew-tasks", user?.ship_id, user?.user_id],
    queryFn: async () => {
      const res = await api.get(
        `/tasks?ship_id=${user?.ship_id || ""}&assigned_to=${user?.user_id || ""}`,
      )
      return res.data
    },
  })
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          ...
          <div className="flex flex-col gap-4">
              ...
              <div key={t.id} className="glass-panel p-6">
                ...
                {t.status !== "Completed" && (
                  <button
                    ...
                    onClick={() =>
                      updateTaskStatus.mutate({ id: t.id, status: "Completed" })
                    }
                  >
            ...
            {tasks.length === 0 && (
              <div className="text-marine-muted bg-marine-card/50 p-6 rounded-xl border border-white/5">
                No tasks assigned.
              </div>
            )}
          </div>
        ...
        <div>
          <h2 className="mb-4 text-marine-cyan font-medium text-lg">
            Upcoming Drills
          </h2>
          <div className="flex flex-col gap-4">
            {drills.map((d: any) => (
              <div key={d.id} className="glass-panel p-6">
                ...
                <button
                  ...
                  disabled={d.attended || markAttendance.isPending}
                  onClick={() => markAttendance.mutate(d.id)}
                >
                  {d.attended
                    ? "Attendance Recorded"
